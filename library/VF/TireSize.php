...
class VF_TireSize
{
    ...
    static function create($formattedString)
    {
        ...
        if (!count($matches)) {
            throw new VF_TireSize_InvalidFormatException('missing section width');
        }
        return new VF_TireSize($matches[self::SECTION_WIDTH], $matches[self::ASPECT_RATIO], $matches[self::OUTSIDE_DIAMETER]);
    }
